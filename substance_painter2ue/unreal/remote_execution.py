...
_TYPE_OPEN_CONNECTION = (
    "open_connection"  # Open a TCP command connection with the requested server (UDP)
)
_TYPE_CLOSE_CONNECTION = (
    "close_connection"  # Close any active TCP command connection (UDP)
)
_TYPE_COMMAND = "command"  # Execute a remote Python command (TCP)
_TYPE_COMMAND_RESULT = (
    "command_result"  # Result of executing a remote Python command (TCP)
)
...
class RemoteExecution(object):
    ...
    def remote_nodes(self):
        ...
        """
        return (
            self._broadcast_connection.remote_nodes
    ...
    def run_command(
        ...
        if raise_on_failure and not data["success"]:
            raise RuntimeError(
                "Remote Python Command failed! {0}".format(data["result"])
            )
        return data
...
class _RemoteExecutionBroadcastConnection(object):
    ...
    def _init_broadcast_socket(self):
        ...
        self._broadcast_socket.bind(
            (
                self._config.multicast_bind_address,
                self._config.multicast_group_endpoint[1],
            )
        )
        self._broadcast_socket.setsockopt(
            _socket.IPPROTO_IP, _socket.IP_MULTICAST_LOOP, 1
        )
        self._broadcast_socket.setsockopt(
    ...
    def _init_broadcast_listen_thread(self):
        ...
        """
        self._broadcast_listen_thread = _threading.Thread(
            target=self._run_broadcast_listen_thread
        )
    ...
    def broadcast_close_connection(self, remote_node_id):
        ...
        self._broadcast_message(
            _RemoteExecutionMessage(
                _TYPE_CLOSE_CONNECTION, self._node_id, remote_node_id
            )
        )
    ...
    def _handle_message(self, message):
        ...
        if message.type_ == _TYPE_PONG:
            ...
            return
        _logger.debug(
            'Unhandled remote execution message type "{0}"'.format(message.type_)
        )
...
class _RemoteExecutionCommandConnection(object):
    ...
    def __init__(self, config, node_id, remote_node_id):
        ...
        self._command_channel_socket = (
            _socket.socket()
        )  # This type is only here to appease PyLint
    ...
    def _init_command_listen_socket(self):
        ...
        if hasattr(_socket, "SO_REUSEPORT"):
            self._command_listen_socket.setsockopt(
                _socket.SOL_SOCKET, _socket.SO_REUSEPORT, 1
            )
        else:
            self._command_listen_socket.setsockopt(
    ...
    def from_json(self, json_str):
        ...
        try:
            ...
            if json_obj["version"] != _PROTOCOL_VERSION:
                raise ValueError(
                    '"version" is incorrect (got {0}, expected {1})!'.format(
                        json_obj["version"], _PROTOCOL_VERSION
                    )
                )
            if json_obj["magic"] != _PROTOCOL_MAGIC:
                raise ValueError(
                    '"magic" is incorrect (got "{0}", expected "{1}")!'.format(
            ...
            _logger.error(
                'Failed to deserialize JSON "{0}": {1}'.format(json_str, str(e))
            )
            return False
        return True
